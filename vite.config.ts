...
export default defineConfig({
  ...
  resolve: {
    ...
    alias: {
      // ✅ 修复：别名无需带版本号，直接映射包名即可（带版本号可能导致依赖解析失败）
      'vaul': 'vaul',
      'sonner': 'sonner',
      'recharts': 'recharts',
      ...
      '@': path.resolve(__dirname, './src'), // ✅ 这个别名是对的，保留
    },
  },
  build: {
    target: 'esnext',
  ...
  server: {
    ...
    host: true,
    proxy: {
      // ✅ 备注：这个代理仅用于本地开发（指向 localhost:3000），生产环境（GitHub Pages）不生效！
      // 生产环境请求直接走 api.ts 里的 https://backend-api-production-6976.up.railway.app
      '/api': {
  ...
  // ✅ 新增：生产环境环境变量（确保 API_BASE_URL 正确指向 6976 域名）
  define: {
    'import.meta.env.VITE_API_BASE_URL': JSON.stringify('https://backend-api-production-6976.up.railway.app'),
  },
});
